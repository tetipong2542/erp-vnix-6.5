...
def normalize_text_key(s: str | None) -> str:
    """ทำ key สำหรับจับคู่ alias แบบทนช่องว่าง/ตัวพิมพ์/จุดพิเศษ"""
    if s is None:
        return ""
    s = str(s).strip()
    s = s.replace("•", " ")
...
def normalize_shop_name(s: str | None) -> str:
    """ทำความสะอาดชื่อร้านสำหรับเก็บ/เทียบ

    - ตัด "(Shopee)" "(Lazada)" ที่พ่วงมา
    - ตัด bullet
    - รวมช่องว่าง
    """
    if s is None:
